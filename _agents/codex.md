...
**Summary:** Streamlined Codex instruction set. Wake-up now auto-checks IM inbox (--agent flag). IM self-messaging triggers added. Session end uses the AIKB stop script.
...
```bash
git -C {{LOCAL_PATH}} pull && python3 {{LOCAL_PATH}}/_tools/memory-pipeline/runtime_cli.py wake-up --agent "Codex CLI"
```
